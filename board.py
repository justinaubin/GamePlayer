...
class Board:
    ...
    def knight(self, space):
        "Returns the space to within range of a knight if there are any"
        spaces = []
        for i in range(-2, 3, 4):
            for j in range(-1, 2, 2):
                ...
                newC = space.col + j
                size = self._size - 1
                if newR >= 0 and newR <= size and newC >= 0 and newC <= size:
                    spaces.append(self._board[newR][newC])
