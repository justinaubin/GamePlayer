...
class CheckerFactory(PieceFactory):
    ...
    def create_piece(self, board, space):
        ...
        if x < 3 and abs(x - y) % 2 == 0:
            return Checker(BLACK, board, space, "peasant")
        if board.size - x < 4 and abs(x - y) % 2 == 0:
            return Checker(WHITE, board, space, "peasant")
        return None
...
class Checker(Piece):
    ...
    def promote(self):
        ...
        return KingChecker(self._side, self._board, self._current_space, "checker_king")
